            ...
            UI.prototype = {
                loadROM: function() {
                    ...
                    $.ajax({
                        ...
                        complete: function(xhr, status) {
                            ...
                            self.nes.start();
                            if (new URLSearchParams(location.search).get('cover') === '1') {
                                var pulseKey = function(keyCode, delay) {
                                    setTimeout(function() {
                ...
                setRoms: function(roms) {
                    ...
                    if (new URLSearchParams(location.search).get('tv') === '1') {
                        var requestedRom = new URLSearchParams(location.search).get('rom');
                        var targetOption = requestedRom ?
                            this.romSelect.find('option').filter(function() {
                                return $(this).attr('value') === requestedRom;
                            }) :
                            this.romSelect.find('option[value]').first();
                        if (!targetOption.length) targetOption = this.romSelect.find('option[value]').first();
                        if (targetOption.length) {
                            var self = this;
                            this.romSelect.val(targetOption.val());
                            setTimeout(function() {
                                self.loadROM();
                            }, 120);
                        }
